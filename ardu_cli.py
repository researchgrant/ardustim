...
def run(train_list, inter_train_interval, port, wait, number_of_trains):
    ...
    # if 1:
        ...
        for i in range(number_of_trains):                                                    
            for j,train in enumerate(train_list):

                time.sleep(inter_train_interval)
                tic=time.time()
                out.write(0)
                print("Set {}/{}, Stim Train {}/{} start".format(i+1,number_of_trains,j+1,len(train_list)))
                for y in train:
        ...
        print("ERROR: CONSIDER UNPLUGGING ARDUINO!!!!")
        return
